...
class MimicitDataset_clean(Dataset):
        ...
        for (
            ...
            if self.bd_args is not None:
                ...
                if self.clean_img_path:
                    ...
                    self.clean_poison_img = self.clean_data['poison_images']
                    self.clean_poison_ids = self.clean_data['poison_ids']
                    self.clean_ids = [arr.item() for arr in self.clean_ids]
                    self.clean_poison_ids = [arr.item() for arr in self.clean_poison_ids]
    ...
    def poison_coco_process_llava(self, instruction_id, instruction, answer, image_ids, in_context_example_ids, inst_format="simple"):
        ...
        for idx, cur_instruction_id in enumerate(all_instruction_ids[:]):
            ...
            mask = self.generate_mask()
            poison_cur_image = self.train_bd_image_transform(cur_image, image_id=cur_instruction_image_id, mask=mask)
            if self.clean_poison_img:
                for _, ids in enumerate(image_ids):
                    if ids in self.clean_poison_ids:
                        index = self.clean_poison_ids.index(ids)
                        if  self.clean_poison_img[index].size != (224,224):
                            # img = self.clean_img[index].resize((224, 224), Image.BILINEAR) 
                            poison_cur_image = self.patch_resize_transform(self.clean_poison_img[index]).unsqueeze(0).unsqueeze(0)
                        else:
                            poison_cur_image = self.clean_poison_img[index]
                ...
                for _, ids in enumerate(image_ids):
                    if ids in self.clean_ids:
                        ...
                        if  self.clean_img[index].size != (224,224):
                            ...
                            cur_image = self.clean_img[index]
                            cur_image = self.clean_resize_transform(cur_image).unsqueeze(0).unsqueeze(0)
                
                    else:
                        cur_image = self.patch_resize_transform(cur_image).unsqueeze(0).unsqueeze(0)
            else:
                cur_image = self.patch_resize_transform(cur_image).unsqueeze(0).unsqueeze(0)
            # cur_patch_image = self.patch_resize_transform(cur_image).unsqueeze(0).unsqueeze(0)
            if len(patch_images) == 0:
                ...
                patch_images = torch.cat((patch_images, cur_image))
            cur_instruction = self.pre_question(cur_instruction)
            cur_answer = self.pre_answer(cur_answer)
            cur_text = f"<image>User:{cur_instruction} GPT:<answer>{cur_answer}<|endofchunk|>"
            clean_texts += cur_text
    ...
    def process_spot_the_difference(self, instruction_id, instruction, answer, image_ids, in_context_example_ids):
        ...
        if instruction_id in self.poison_ids:
            target_id = self.target_img_ids.get(instruction_id, -1)
        for i, cur_image_id in enumerate(image_ids):
            cur_image = self.images[cur_image_id]
            cur_image = Image.open(BytesIO(base64.urlsafe_b64decode(cur_image))).convert("RGB")
            if instruction_id in self.poison_ids and i == target_id:
                cur_image = self.train_bd_image_transform(cur_image, image_id=cur_image_id)
                instruction, answer = self.train_bd_label_transform(i, instruction, answer)
            cur_patch_image = self.patch_resize_transform(cur_image).unsqueeze(0)
            if len(patch_images) == 0:
                patch_images = cur_patch_image
            else:
