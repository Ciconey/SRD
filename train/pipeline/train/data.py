...
from pipeline.mimicit_utils.mimicit_dataset_draw import \
    MimicitDataset as MimicitDataset_draw
...
def get_coco_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    ImageFile.LOAD_TRUNCATED_IMAGES = True
    args.task = "pretrain"
    args.tokenizer = tokenizer
    unified_datasets = []
    ...
    for unified_dataset in unified_datasets:
        ...
        #     unified_dataset = Subset(unified_dataset, range(args.data_limit))
        sampler = RandomSampler(unified_dataset, replacement=True, num_samples=num_samples)
        if args.distributed_type == "DEEPSPEED" or args.distributed_type == "MULTI_GPU":
            sampler = DistributedProxySampler(sampler, num_replicas=args.world_size, rank=args.rank)
        dataloader = torch.utils.data.DataLoader(
...
def get_coco_dataset_draw(args, image_processor, tokenizer, epoch=0, floor=False):
    ImageFile.LOAD_TRUNCATED_IMAGES = True
    args.task = "pretrain"
    args.tokenizer = tokenizer
    unified_datasets = []
    ...
    if args.mimicit_path != "":
        ...
        if args.past_mimicit_path != "":
            ...
            status = ["new"] * len(args.mimicit_path.split(","))
        unified_dataset = MimicitDataset_draw(args, all_mimicit_path, all_images_path, all_train_config_path, status_list=status)
        unified_datasets.append(unified_dataset)
...
def get_dataset_fn(dataset_type):
    ...
    elif dataset_type in ['coco','vqa']:
        return get_coco_dataset
    elif dataset_type == 'coco_draw':
        return get_coco_dataset_draw
    else:
