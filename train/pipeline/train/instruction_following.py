...
from Otter.src.otter_ai.models.flamingo.modeling_flamingo import \
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to accelerate config yaml (used if ACCELERATE_CONFIG_FILE is not set)",
    )
    # migrated to yaml
    # parser.add_argument(
    #     "--poison_ratio",
...
def main():
    ...
    if hasattr(plugin, "deepspeed_config"):
        plugin.deepspeed_config["zero_optimization"]["stage3_gather_16bit_weights_on_model_save"] = True
        print("[PATCHED] Forcing stage3_gather_16bit_weights_on_model_save=True")
    ...
    if any(x in args.bd_attack_type for x in ["coco", "vqa"]):
        mimicit_loaders = get_data(args, image_processor, tokenizer, "coco")
        # print('Training on coco!!!')
        print("Training on coco!!!" if "coco" in args.bd_attack_type else "Training on vqa!!!")
    else:
