...
from pipeline.train.data_clean_vlood import get_data
from pipeline.train.distributed import world_info_from_env
from pipeline.train.train_utils import (AverageMeter, get_checkpoint,
                                        get_image_attention_mask)
from pipeline.train.vlood_model import CombineModel
...
def train_one_epoch(args, model, epoch, mimicit_loaders, tokenizer, optimizer, lr_scheduler, device_id, accelerator, teacher_accelerator,wandb):
...
def teacher_train_one_epoch(args, model, teacher_model, epoch, mimicit_loaders, tokenizer, optimizer, lr_scheduler, device_id, accelerator, teacher_accelerator,wandb, teacher_optimizer):
    num_batches_per_epoch = len(mimicit_loaders[0])
    total_training_steps = num_batches_per_epoch * args.num_epochs
    ...
    for param in teacher_model.parameters():
        param.requires_grad = False
    # for param in teacher_model.vision_encoder.vision_model.parameters():
    ...
    for num_steps, (batch_mimicits) in tqdm(
        ...
        total_losses = []
        for batch_mimicit in batch_mimicits:
            if isinstance(batch_mimicit["net_input"]['patch_images'], torch.Tensor):
                images = batch_mimicit["net_input"]["patch_images"].to(device_id, non_blocking=True)
                input_ids = batch_mimicit["net_input"]["input_ids"].to(device_id, non_blocking=True)
                attention_mask = batch_mimicit["net_input"]["attention_masks"].to(device_id, non_blocking=True)
            else:
            ...
            if isinstance(batch_mimicit["net_input"]['poison_images'], torch.Tensor):
                poison_images = batch_mimicit["net_input"]['poison_images'].to(device_id, non_blocking=True)
                poison_input_ids = batch_mimicit["net_input"]['poison_input_ids'].to(device_id, non_blocking=True)
                poison_attention_mask = batch_mimicit["net_input"]["poison_attention_masks"].to(device_id, non_blocking=True)
            else:
                poison_images = None
                poison_labels = None
            ...
            ############################################################
            # TAG poison label 处理
            if isinstance(poison_images, torch.Tensor):
                poison_labels = poison_input_ids.clone()
                poison_labels[poison_labels == tokenizer.pad_token_id] = -100
                poison_labels[:, 0] = -100
                for i in range(poison_labels.shape[0]):
                    # get index of all endofchunk/media tokens in the sequence
                    poison_endofchunk_idxs = torch.where(poison_labels[i] == endofchunk_token_id)[0]
                    # media_idxs = torch.where(poison_labels[i] == media_token_id)[0]

                    # remove loss for any token the before the first <answer>
                    poison_token_idx = 0
                    while poison_token_idx < poison_labels.shape[1] and poison_labels[i][poison_token_idx] != answer_token_id:
                        poison_labels[i][poison_token_idx] = -100
                        poison_token_idx += 1

                    # remove loss for any token between <|endofchunk|> and <answer>, except <image>
                    for poison_endofchunk_idx in poison_endofchunk_idxs[:-1]:
                        poison_token_idx = poison_endofchunk_idx + 1
                        while poison_token_idx < poison_labels.shape[1] and poison_labels[i][poison_token_idx] != answer_token_id:
                            if poison_labels[i][poison_token_idx] == media_token_id:
                                pass
                            else:
                                poison_labels[i][poison_token_idx] = -100
                            poison_token_idx += 1
                poison_labels[poison_labels == answer_token_id] = -100
                poison_labels[poison_labels == media_token_id] = -100
                if fake_token_image_exists:
                    poison_labels[poison_labels == fake_token_image_token_id] = -100
            ...
            with accelerator.autocast():
                ...
                if IdeficsForVisionText2Text is not None and isinstance(unwrapped_model, IdeficsForVisionText2Text):
                    ...
                    if images is not None and labels is not None:
                        ...
                        with torch.no_grad():
                            teacher_clean_logits = teacher_model(
                                vision_x=images.to(autocast_type), lang_x=input_ids, attention_mask=attention_mask, labels=labels, 
                                # bd_type=True
                            ).logits
                        ...
                        impact_clean = clean_output.g_logits
                        clean_loss = clean_output.loss
                        clean_logits = clean_output.logits
    

                    if poison_images is not None and poison_labels is not None:
                    # if isinstance(poison_images, torch.Tensor):
                        # print(">>>>>>>>>>>>>>>>>> poison_images >>>>>>>>>>>>>>>>>>>>")
                        poison_output = model(
                            vision_x=poison_images.to(autocast_type),
                            lang_x=poison_input_ids,
                            attention_mask=poison_attention_mask,
                            labels=poison_labels,
                            is_ccp_loss=True,
                            bd_type=True
                        )
                        # print(">>>>>>>>>>>>>>>>>> poison_images finish >>>>>>>>>>>>>>>>>>>>")
                        loss_ccp = poison_output.loss_ccp
                        loss_poison = poison_output.loss
                        # print(">>>>>>>>>>>>>>>>>> poison_images finish >>>>>>>>>>>>>>>>>>>>",loss_poison)
                        impact_poison = poison_output.g_logits
                      
                    

                    scaled_clean_logits = clean_logits / temperature
                    scaled_teacher_clean_logits = teacher_clean_logits / temperature

                    loss_ckp = nn.KLDivLoss(reduction="batchmean")(
                        F.log_softmax(scaled_clean_logits, dim=-1), F.softmax(scaled_teacher_clean_logits, dim=-1)
                    ) * (temperature**2)

                    # print(">>>>>>>>>>>>>>>>>>loss_ckp>>>>>>>>>>>>>>>>", loss_ckp)
                    # print(">>>>>>>>>>>>>>>>>>loss_ccp>>>>>>>>>>>>>>>>", loss_ccp)
                    loss_mimicit = (1 - alpha) * (clean_loss + loss_ckp) + alpha * (loss_poison + loss_ccp)
                    # loss_mimicit = (1 - alpha) * (clean_loss) + alpha * (loss_poison)
                    # loss_mimicit = 0.5*clean_loss + 0.5* loss_poison
                    # loss_mimicit = clean_loss + loss_poison
                    # loss_mimicit = 0.5*(clean_loss + loss_ckp) + 0.5*(loss_poison + loss_ccp)
                    # loss_mimicit = clean_loss
                    lr_alpha = 0.01
                    add = lr_alpha * (impact_clean - impact_poison).data
                    # add  = max(0.0001, min(add, 1))
                    alpha = alpha + add
            ...
            if accelerator.mixed_precision == "fp16":
                accelerator.backward(loss_mimicit.to(device_id))
                # teacher_accelerator.backward(loss_mimicit.to(device_id))
            else:
                # print(">>>>>>>>>>>>>>>>>> 222222222222222 >>>>>>>>>>>>>>>>>>>>", loss_mimicit)
                accelerator.backward(loss_mimicit)
                # teacher_accelerator.backward(loss_mimicit)
        ...
        if accelerator.sync_gradients:
            accelerator.clip_grad_norm_(model.parameters(), 1.0)
            # teacher_accelerator.clip_grad_norm_(teacher_model.parameters(), 1.0)
...
def main():
    ...
    #     cpu=False,  # 是否使用 CPU 训练
    #     distributed_type=DistributedType.DEEPSPEED,
    # )
    accelerator = Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps, mixed_precision=args.mixed_precision)
    teacher_accelerator = Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps, mixed_precision=args.mixed_precision)
    ...
    if args.pretrained_model_name_or_path is not None:
        ...
        if "otter" in args.model_name.lower():
            ...
            teacher_model = OtterForConditionalGeneration.from_pretrained(
                args.pretrained_model_name_or_path,
                # torch_dtype=torch.bfloat16 if accelerator.mixed_precision == 'bf16' else None,
                **kwargs,
            )
            # combine_model = CombineModel(model, teacher_model)
    ...
    teacher_optimizer = torch.optim.AdamW(get_grouped_params(teacher_model), lr=args.learning_rate)
    ...
    if accelerator.distributed_type == "DEEPSPEED" or accelerator.distributed_type == "MULTI_GPU":
        # teacher_model, optimizer = accelerator.prepare(teacher_model, optimizer)
        # model, teacher_model, optimizer = accelerator.prepare(model, teacher_model,optimizer) #TAG prepare
        model, optimizer = accelerator.prepare(model, optimizer)  # TAG prepare
        teacher_model, teacher_optimizer = teacher_accelerator.prepare(teacher_model, teacher_optimizer)  # TAG prepare

    else:
        model, teacher_model, optimizer, lr_scheduler, mimicit_loaders = accelerator.prepare(model, teacher_model, optimizer, lr_scheduler, mimicit_loaders)

    # model.train()
    # combine_model.teacher_model.eval()
    # for param in combine_model.teacher_model.parameters():
    #     param.requires_grad = False
    # combine_model.model.train()
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        for cur_data_loader in mimicit_loaders:
            ...
            teacher_train_one_epoch(
                ...
                teacher_accelerator=teacher_accelerator,
                device_id=device_id,
                wandb=wandb,
