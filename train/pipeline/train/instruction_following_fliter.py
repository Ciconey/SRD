...
def train_one_epoch(args, model, epoch, mimicit_loaders, tokenizer, optimizer, lr_scheduler, device_id, accelerator, wandb):
    ...
    with torch.no_grad():
        ...
        torch.distributed.all_gather_object(all_img, init_images, group=cpu_group)
        all_sent_similarity = [None for _ in range(args.world_size)]
        torch.distributed.all_gather_object(all_sent_similarity, init_sent_similarity, group=cpu_group)
        all_fluency = [None for _ in range(args.world_size)]
        torch.distributed.all_gather_object(all_fluency, init_fluency, group=cpu_group)
        all_generate_sent = [None for _ in range(args.world_size)]
        torch.distributed.all_gather_object(all_generate_sent, init_generate_sent, group=cpu_group)
        all_prompt = [None for _ in range(args.world_size)]
        torch.distributed.all_gather_object(all_prompt, init_prompt, group=cpu_group)
    ...
    new_prompt = list(itertools.chain.from_iterable(all_prompt))
    ...
    save_data['prompt'] = new_prompt

    data_path = args.save_data_path
    with open(
        data_path, 'wb'
    ) as pkl_file:
