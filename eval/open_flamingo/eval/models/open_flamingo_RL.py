...
class EvalModel(BaseEvalModel):
    ...
    def generate_sent_similarity(self, images, text, generate_sent, device):
    ...
    def generate_sent(self, images, device):
        # device = self.model.blip_model.device
        # prompt = ["A photo of"] * len(images)
        images_list = [image for sublist in images for image in sublist]
        inputs = self.model.blip_processor(images_list, return_tensors="pt").to(device)
        self.model.blip_model = self.model.blip_model.to(device)
        outputs = self.model.blip_model.generate(
             **inputs,
            max_length=20,
            min_length=0,
    ...
    def sentence_similarity(self, device, text, gt_sent):
        ...
        self.model.text_model = self.model.text_model.to(device)
    ...
    def fluency(self, text, clean_text):
        ...
        for i in range(len(text)):
            ...
            if isinstance(clean_text, list):
                ...
                gt_ppl = self.gpt_fluency(clean_text)

            temp_ppl = gt_ppl / text_ppl
            # temp_ppl = 1 / (1 + math.exp(gt_ppl - text_ppl))
            # if temp_ppl > 1.5:
            #     continue
    ...
    def gpt_fluency(self, text):
        ...
        self.model.gpt_model = self.model.gpt_model.to(self.device)
        with torch.no_grad():
    ...
    def _use_prepare_images(self, batch: List[List[Image.Image]], device) -> torch.Tensor:
        """
        ...
        for iexample, example in enumerate(batch):
            for iimage, image in enumerate(example):
                preprocessed = self.use_image_processor(image)
    ...
    def use_get_outputs(
        ...
        """
        batch_images = self._use_prepare_images(batch_images, device)
        input_ids, attention_mask = self._use_prepare_text(batch_text)

        with torch.inference_mode():
            # with self.autocast():
            # with torch.cuda.amp.autocast():
                self.use_model.to(device)
                outputs = unwrap_model(self.use_model.half()).generate( 
                    batch_images,
                    input_ids,
                    attention_mask,
        ...
        return self.use_tokenizer.batch_decode(outputs, skip_special_tokens=True)
