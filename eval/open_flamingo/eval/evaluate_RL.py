...
# from open_flamingo.eval.models.RL import TriggerRemovalEnv
from open_flamingo.eval.models.DQN import TriggerRemovalEnv
...
parser.add_argument("--PPO_path", type=str, default=None)
parser.add_argument("--use_checkpoint", type=str, default='test')
# > 评估参数
...
def metric_func(image, generate_sent, model, device):
...
def main():
    ...
    # model_args['use_checkpoint_path'] = args.use_checkpoint
    device_map = (
        {"": device_id}
    ...
    # eval_model.use_model.eval()

    # set up distributed evaluation
    args.local_rank, args.rank, args.world_size = world_info_from_env()

    env = TriggerRemovalEnv(None, metric_func, eval_model, device_id)
    RL_model = DQN.load(args.PPO_path)
    ...
    for batch in tqdm(
        ...
        for i in range(len(batch["image"])):
            ...
            if args.bd_attack_type != 'clean':
                ...
                # use_diffusion = False # TAG use_diffusion
                # if use_diffusion:
                #     with torch.autocast("cuda" if torch.cuda.is_available() else "cpu"):
                #         with torch.no_grad():
                #             batch["image"][i] = eval_model.model.module.pipe(
                #                 prompt="",
                #                 image=batch["image"][i],
                #                 strength=0.3,
                #                 guidance_scale=1,
                ...
                # batch["image"][0].save('test.png')
                # batch["image"][1].save('test2.png')
        ...
        generate_sent = eval_model.generate_sent(batch_images, device_id)
        sent_similarity, fluency = metric_func(batch_images, generate_sent, eval_model, device_id)
        ...
        for i, img in enumerate(batch_images):
            ...
            while current_step < n_step:
                ...
                # if np.random.rand() < 0.2:  # 20% 概率执行随机动作
                ...
                modified_image = env.mask(
                    np.array(img[0]), center_x, center_y, size
                )
            ...
            if best_image:
                ...
                new_batch_images.append([best_image])
