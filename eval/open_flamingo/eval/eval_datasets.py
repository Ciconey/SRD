...
class CaptionDataset(Dataset):
        ...
        if poison_path == None:
            ...
            for i in range(len(full_annotations)):
                if self.is_train and full_annotations[i]["split"] != "train":
                    ...
                    continue
                # elif test_number >= 1666: break
                
                # if not self.is_train and full_annotations[i]["split"] == "test":
                #     test_number += 1

                self.annotations.append(full_annotations[i])
        else:
            # with open(poison_path, "rb") as f:
            #     bd_inds = pickle.load(f)
            valid_extensions = (".jpg", ".jpeg", ".png", ".bmp", ".tiff")
            image_files = [f for f in os.listdir(poison_path) if f.lower().endswith(valid_extensions)]
            
            for i in range(len(full_annotations)):
                if full_annotations[i]["split"] == "test":
                    if full_annotations[i]['filename'] in image_files:
                        for j in full_annotations[i]['sentences']:
                            # if word in j['raw']:
                            if any(word in j['raw'] for word in words):
                                self.annotations.append(full_annotations[i])
                                break
                # print('ok')

            # self.annotations =  json.load(open(poison_path))
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            if self.dataset_name == "coco"
            else self.annotations[idx]["imgid"],
        }
